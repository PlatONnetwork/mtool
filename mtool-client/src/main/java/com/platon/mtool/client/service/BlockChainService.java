...
public class BlockChainService {
    ...
    public void setContractUtil(ContractUtil contractUtil) {
    ...
    public void validAddressNotBeenUsed(
        ...
        NodeContract nodeContract = NodeContract.load(web3j);
    ...
    public void validBalanceEnough(
            String address, BigInteger amount, GasProvider gasProvider, Web3j web3j, StakingAmountType amountType) throws Exception {
        // 总花费 = 交易金额 + 手续费
        BigInteger total = getCostAmount(amount, gasProvider);
        BigInteger fee = total.subtract(amount);

        // 钱包自由余额
        BigInteger freeBalance = web3j.platonGetBalance(address, DefaultBlockParameterName.LATEST).send().getBalance();
        switch (amountType) {
            ...
            case RESTRICTING_AMOUNT_TYPE:
                // 钱包锁仓可用余额
                BigInteger restrictBalance = queryRestrictingBalanceAvailableForStakingOrDelegation(address, web3j);
            ...
            case AUTO_AMOUNT_TYPE:
                // 钱包锁仓可用余额
                restrictBalance = queryRestrictingBalanceAvailableForStakingOrDelegation(address, web3j);
    ...
    public BigInteger queryRestrictingBalanceAvailableForStakingOrDelegation(String userAddress, Web3j web3j) throws Exception {
        ...
        RestrictingPlanContract restrictingPlanContract = RestrictingPlanContract.load(web3j);
    ...
    public void validAmount(Web3j web3j, String module, String paramName, BigInteger amount)
        ...
        if (!response.isStatusOk()) {
            throw new MtoolClientException("Request target chain[" + CLIENT_CONFIG.getChainId() + "] error");
        }

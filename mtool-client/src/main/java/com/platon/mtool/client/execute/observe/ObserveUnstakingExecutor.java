 ...
 * <p>Created by liyf.
 */
public class ObserveUnstakingExecutor extends MtoolExecutor<UnstakingOption> {

  private static final Logger logger = LoggerFactory.getLogger(ObserveUnstakingExecutor.class);
  private BlockChainService blockChainService = BlockChainService.singleton();

  public ObserveUnstakingExecutor(JCommander commander, UnstakingOption commonOption) {
    super(commander, commonOption);
  }

  @Override
  public void execute(UnstakingOption option) throws Exception {
    LogUtils.info(logger, () -> Log.newBuilder().msg("Unstaking").kv("option", option));
    ProgressBar.start();
    ...
    entity.setType(FuncTypeEnum.UNSTAKING);
